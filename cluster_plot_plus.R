...
da <- read.table(otu, header=T, row.names=1, check.names=FALSE, sep='\t', comment.char="", quote="", skip=1)
...
p_bar <- ggplot(data=b) +
geom_bar(aes(y=value, x=variable, fill=species), position = "stack", width=0.45, stat="identity") +
theme_bw() +
xlab("") + 
blank_theme +
ylab("") +
