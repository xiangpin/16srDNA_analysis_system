...
envFactor <- tmp
envFactor <- data.frame(t(envFactor))
#head(otuTable, 2)
dim(otuTable)
#head(envFactor, 2)
dim(envFactor)
...
p <- ggplot(data=x, aes(RDA1, RDA2, group)) + 
geom_segment(data=env, aes(x=0, y=0, xend=RDA1, yend=RDA2), arrow=arrow(length=unit(0.3, "cm")),size=0.8, colour="azure3")+
geom_point(aes(color=group,shape=group),size=2) +
scale_color_manual(values=mycolors)+
scale_shape_manual(values=shapes)+
theme_bw() +
geom_segment(aes(x=gg$xcenter, y=gg$ycenter, xend=gg$RDA1, yend=gg$RDA2, color=gg$group), size=0.3)+
xlab(xlab_text)+
ylab(ylab_text)+
#geom_segment(data=env, aes(x=0, y=0, xend=RDA1, yend=RDA2), arrow=arrow(length=unit(0.3, "cm")),size=0.8, colour="")+
...
coord_flip()+
theme_bw() + 
theme(panel.grid = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1),axis.text.y = element_text(angle = 50, hjust = 1, face="bold" ),legend.position="none",plot.title = element_text(face="bold",lineheight=25,hjust=0.5))

p3 <- ggplot(data = x) + 
geom_boxplot(aes(x=group, y=RDA2,fill=group),outlier.size=0,outlier.shape=NA)+
scale_fill_manual(values=mycolors)+
xlab("")+
ylab("")+
theme_bw() + 
theme(panel.grid = element_blank(),panel.border = element_rect(colour = "black", fill=NA, size=1),axis.text.x = element_text(angle=40, hjust=1,face="bold"),legend.position="none")
...
cca_p <- ggplot(data=x, aes(CCA1, CCA2, group)) + 
geom_segment(data=env, aes(x=0,y=0, xend=CCA1, yend=CCA2), arrow=arrow(length=unit(0.3, "cm")),size=0.7,colour="azure3")+
geom_point(aes(color=group,shape=group),size=2) +
scale_color_manual(values=mycolors) +
scale_shape_manual(values=shapes) +
theme_bw() +
geom_segment(aes(x=gg$xcenter, y=gg$ycenter, xend=gg$CCA1, yend=gg$CCA2,color=gg$group), size=0.3) +
xlab(x_lab) +
ylab(y_lab) +
...
cca_p3 <- ggplot(data = x) + 
geom_boxplot(aes(x=group, y=CCA2,fill=group),outlier.size=0,outlier.shape=NA)+
scale_fill_manual(values=mycolors)+
xlab("")+
ylab("")+
theme_bw() + 
