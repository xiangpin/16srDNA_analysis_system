...
chimerada <- read.table(chimera, sep="\t", header=T, row.names=1, check.names=F)
otutabda <- read.table(otutab, sep="\t", header=T, row.names=1, check.names=F)
j = 1
otu <- data.frame()
...
dt$Row.names <- NULL
colnames(dt) <- c("RawReadsNums", "CleanReadsNums")
